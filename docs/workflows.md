...
- `extract_assessable_sections(...)` followed by `iter_assessment_steps(...)`
  for Assessor mode;
- `iter_applicant_assessment_steps(...)` for Applicant mode.
...
The Applicant bundle contains the applicant-facing GOG, Application Form and
...
The frontend already knows its form fields, so Applicant mode does not run
extraction.

1. The frontend submits only fields marked for AI review.
2. The backend validates and orders those fields.
3. Each field is sent in a separate Gemini request with the Applicant document
   bundle.
4. Results are returned under the field IDs for inline display.

No Applicant call receives another draft answer. A vague budget label therefore
cannot be silently resolved using an activity description.

The attachment checklist is submitted as one isolated text manifest containing
only each listed attachment name and what it represents. GOGgles compares that
manifest with the supplied mandatory requirements. It receives no attachment
file, cannot authenticate or inspect one, and does not use other draft answers to
infer whether a conditional attachment applies.

The frontend uses `POST /api/applicant/check/stream`. It reports one real stage per
field followed by the normal `ApplicantCheckResult`.
...
Applicant budget checks have a stricter postcondition because each draft field is
...
synthetic applications and Applicant smoke checks.
